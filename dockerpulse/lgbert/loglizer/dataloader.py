...
def load_HDFS(log_file, label_file=None, window='session', train_ratio=0.5,
        ...
        if window_size > 0:
            ...
            print(
                log.format(
                    ...
                    (1 - y_test).sum(),
                    y_test.shape[0]))
            return (x_train, window_y_train,
                    y_train), (x_test, window_y_test, y_test)
...
def slice_hdfs(x, y, window_size):
    results_data = []
    print(
        "Slicing {} sessions, with window {}".format(
            x.shape[0],
            window_size))
    ...
    return results_df[["SessionId", "EventSequence"]
                      ], results_df["Label"], results_df["SessionLabel"]
...
def bgl_preprocess_data(para, raw_data, event_mapping_data):
    ...
    # create the directory for saving the sliding windows (start_index,
    # end_index), which can be directly loaded in future running
    ...
    # list of tuples, tuple contains two number, which represent the start and
    ...
    # get all the log indexes in each time window by ranging from start_index
    # to end_index
